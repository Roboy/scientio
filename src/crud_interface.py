...
class CrudInterface(object):
    # Create a node
    @staticmethod
    def create(request: memory_node_model.MemoryNodeModel):
    ...
    # Get node by ID
    @staticmethod
    def retrieve(request: memory_node_model.MemoryNodeModel, node_id, memory: memory_interface.MemoryInterface):
        if node_id > 0:
    ...
    # Update Nodes
    @staticmethod
    def update(request: memory_node_model.MemoryNodeModel):
        if not request.get_label().__eq__(neo4j_label.Neo4jLabel().other) and \
                not request.get_label().__eq__(neo4j_label.Neo4jLabel().none):
    ...
    # Delete a Node
    @staticmethod
    def delete(request: memory_node_model.MemoryNodeModel, memory: memory_interface.MemoryInterface):
